...
def compare_values(x, y, rtol=1e-4, atol=1e-8):
    """
    ...
    if isinstance(y, (int, float)):
        try:
            ...
            if passed:
                msg = f"value {x:.6g} is correct within tolerance."
            else:
                err, rel_err = get_deviation(x, y, rtol=rtol, atol=atol)
                msg = f"value produced: {x:.6g}. <br>absolute error: {err:.6g} <br>relative error: {rel_err:.6g}."
        ...
        if len(x) != len(y):
            ...
            try:
                ...
                if passed:
                    ...
                    err, rel_err = get_deviation(x, y, rtol=rtol, atol=atol)
                    msg = f"too large mismatch for array. <br>max abslute error: {err:.6g} <br>max relative error = {rel_err:.6g}."
